   ...
   Dropzone.options.NRICBackDZ = {
        ...
        init: function() {
            ...
            this.on("uploadprogress", function(file) {
                ...
                html += '</div>';
                html += '</div>';
                $('#NRICBackDZ .dz-message').html(html).show();
            });
            this.on("maxfilesreached", function(file) {});
            ...
            this.on("success", function(file, response) {
                ...
                $('#NRICBackDZ .dz-message').html(html).show();
                setTimeout(function() {
                    $('#NRICBackDZ .dz-message').html('<span style="position: relative;top: -44px;right: -20px;"><img src="{{MEDIADOMAIN}}/images/camera.png"></span>').show();
                }, 2000);
                $('.NRICBack').attr('src', '/uploads/users/id/{{ $user->id }}/'+file.upload.filename+'?' + new Date().getTime());
            });
    ...
    var NRICBackDZ = new Dropzone("#NRICBackDZ");
});
</script>