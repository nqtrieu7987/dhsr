...
$(function() {
   Dropzone.options.studentCardFrontDZ = {
        paramName: 'file',
        maxFilesize: 10, // MB
        addRemoveLinks: true,
        maxFiles: 1,
        ...
        init: function() {
            ...
            this.on("uploadprogress", function(file) {
                ...
                html += '</div>';
                html += '</div>';
                $('#studentCardFrontDZ .dz-message').html(html).show();
            });
            this.on("maxfilesreached", function(file) {});
            ...
            this.on("success", function(file, response) {
                ...
                $('#studentCardFrontDZ .dz-message').html(html).show();
                setTimeout(function() {
                    $('#studentCardFrontDZ .dz-message').html('<span style="position: relative;top: -44px;right: -20px;"><img src="{{MEDIADOMAIN}}/images/camera.png"></span>').show();
                }, 2000);
                $('.studentCardFront').attr('src', '/uploads/users/id/{{ $user->id }}/'+file.upload.filename+'?' + new Date().getTime());
            });
    ...
    var studentCardFrontDZ = new Dropzone("#studentCardFrontDZ");
});
</script>