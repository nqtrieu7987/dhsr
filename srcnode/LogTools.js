...
function genLogSecondFoler(s) {
    return FOLDER_LOG + 'edupiasecondary/' + Math.abs(hashCode(s) % 1024) + '/'
}
...
module.exports = {
    ...
    getLogSecondByUser: function getLogByUser(req, res) {
        var username = req.headers.username;
        res.setHeader('Content-Type', 'application/json');
        if (req.headers.username == undefined || utils.isEmptyObject(req.headers.username)) {
            res.json({ message: 'Input username error!', resultCode: 0 });
        ...
        redis.hget(config.keyPhone2User, username, function (err, obj) {
            ...
            var folderName = genLogSecondFoler(username) + username;
            try {
                fs.readdir(folderName, (err, files) => {
                    res.json(files);
                })
    ...
    getAllUserLogSecond: function getAllUserLog(req, res) {
        var username = req.headers.username;
        var date = req.headers.datetime;
        //utils.writeLog("Get log for user:" + username + " date: " + date);
        res.setHeader('Content-Type', 'application/json');
        ...
        redis.hget(config.keyPhone2User, username, function (err, obj) {
            ...
            var fileName = genLogSecondFoler(username) + username + "/" + date;
            try {
                var file = fs.readFileSync(fileName, "utf8");
                var array = file.split("\n");
                res.json(array);
