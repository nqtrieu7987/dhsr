...
async function getMyJob(req, res) {
    ...
    if (status == null) {
        var sql =
            `SELECT ds_job.id, ds_job.hotel_id, ds_hotel.name AS hotel_name, ds_job.is_active,ds_all_jobs.status AS STATUS, ds_job.start_date, ds_job.start_date, ds_job.job_type_id, ds_job_type.name, ds_job.start_time, ds_job.end_time, DATE_FORMAT(start_date, '%d %b') AS day_month, DATE_FORMAT(start_date, '%W') AS  weekyday 
        FROM ds_all_jobs 
        LEFT JOIN ds_job ON ds_all_jobs.job_id = ds_job.id 
        LEFT JOIN ds_hotel ON ds_job.hotel_id = ds_hotel.id 
        LEFT JOIN ds_job_type ON ds_job.job_type_id = ds_job_type.id 
        ...
        LEFT JOIN ds_hotel ON ds_job.hotel_id = ds_hotel.id 
        LEFT JOIN ds_job_type ON ds_job.job_type_id = ds_job_type.id  
        WHERE ds_all_jobs.user_id=${user_id} AND ds_all_jobs.status=${status} 
        ORDER BY ds_job.start_date DESC`;
    }
