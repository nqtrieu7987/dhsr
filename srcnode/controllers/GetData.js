...
async function getListJobOnGoing(req, res) {
    ...
    if (req.headers.hotel_id == undefined || utils.isEmptyObject(req.headers.hotel_id)) {
        ...
        var sql = `SELECT ds_job.id, ds_job.hotel_id, ds_hotel.name AS hotel_name, ds_job.is_active, ds_job.start_date, ds_job.job_type_id, ds_job_type.name, ds_job.start_time, ds_job.end_time, DATE_FORMAT(start_date, '%d %b') AS day_month, DATE_FORMAT(start_date, '%W') AS  weekyday, slot
        FROM ds_job INNER JOIN ds_hotel ON ds_hotel.id = ds_job.hotel_id 
        INNER JOIN ds_job_type ON ds_job.job_type_id = ds_job_type.id 
