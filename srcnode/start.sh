APP_RUN_DIR=/home/educa/edupia.vn/nodejs/service/run
...
start)
    ...
    while :		
            ...
            if (( $(ps -ef | grep -v grep | grep $service | grep serve.js | wc -l) > 0 ))
            then
                echo "$(date) : $service is running!!!" >>$LOG_FILE
            else
                node serve.js 1>>logservice.log 2>>logservice.error &
